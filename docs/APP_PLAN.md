...
1. 当前调试包通过 USB 转发访问本机 API；手机到 Docker 的局域网健康接口和带会话账本读取已验证。手机浏览器已通过 `http://10.141.156.100:8000/` 完成局域网同源验收；下一阶段核对电脑浏览器、手机浏览器和 APK 的同账号数据，再配置正式 HTTPS API 地址，不能把 `127.0.0.1` 或局域网地址写进发布包。
2. 验证 Android WebView 的 Cookie、CORS、会话过期和失败提示。
3. 用同一账号在电脑浏览器、手机浏览器和 APK 之间核对账本、账目和净资产；登录后可使用“同步”按钮主动读取另一端的新数据。
