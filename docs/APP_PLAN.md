...
个人离线 APK 已生成并安装。下一轮手机端体验、图标和家人更新方案见 [`UI_EVOLUTION_PLAN.md`](UI_EVOLUTION_PLAN.md)。

## 当前确定范围（2026-09-18）

当前版本只验收“个人手机离线使用”：安装 APK 后，在没有服务器、域名、Docker、USB 连接和网络的情况下，也能打开记账本、保存账目、查看统计并导入导出 JSON 备份。

- 数据保存在当前手机的 App 本地存储中；换手机或清除 App 数据不会自动带来原设备账目，需要通过备份文件迁移。
- 服务器、登录、跨设备同步和移动网络访问属于后续可选扩展，不是个人离线 APK 的使用前提。
- 当前调试 APK 可以继续用于真机验收；后续优先整理可重复生成的个人离线 APK，再考虑签名和发布渠道。
...
- 电脑浏览器、手机浏览器和 APK 共用同一套前端业务代码；个人离线 APK 直接使用 App 本地 localStorage，服务端同步需要可从手机移动网络访问的 HTTPS 地址，属于后续扩展。
...
这部分保留为后续路线，不阻塞个人离线 APK。当前的 USB 转发、局域网地址和 HTTPS 预检主要用于学习后端部署与跨设备同步；个人使用时可以完全跳过。
...
- 个人离线模式不依赖后端：数据直接保存到手机本地 localStorage，导出 JSON 是跨设备迁移的主要方式。带服务端配置的调试包在网络不可用时也会自动回退到“本机离线模式”；真正的联网登录和同步仍需要服务器。
- 不把移动网络同步、邮箱验证码、服务器部署和应用商店上架作为当前版本验收条件。
