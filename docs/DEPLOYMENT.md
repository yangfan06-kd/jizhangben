...
项目提供了自动化恢复检查脚本。它只创建临时卷，验证完成后自动删除，不会覆盖正式卷：

```powershell
./deploy/restore-check.ps1 -BackupPath "G:\jizhangben-backups\jizhangben-20260918-133535.db"
```

脚本会检查 SQLite 完整性、schema 版本以及用户、账本和账目数量；正式服务器可以把它安排在独立恢复机或临时维护窗口中执行。
