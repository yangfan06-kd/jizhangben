...
2. 后端在一个数据库事务中创建本地开发用户。
3. 导入账本，建立旧账本 ID 到新 UUID 的映射。
4. 导入并去重类别、内置类型和自定义类型。
5. 按账本导入账户，建立旧账户 ID 到新 UUID 的映射。
6. 第一遍导入账目，暂不写入转入账户和押金关联。
7. 第二遍根据映射补齐 `to_account_id` 和 `deposit_link_id`。
8. 对比迁移前后的账本数、账户数、账目数、收入、支出和净资产。
9. 全部一致后提交事务；任何一步失败都回滚。
10. 前端确认服务端数据可读取前，继续保留 localStorage 和原备份，不自动清除。
...
接下来让网页数据访问层逐步接入后端，并保留 localStorage 回退与迁移演练。
